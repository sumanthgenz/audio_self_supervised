...
class VideoFeatureModel(torch.nn.Module):
    ...
    def forward(self, input_video):
        ...
        video_frames = input_video.reshape(v.shape[0]*v.shape[2], v.shape[1], v.shape[3], v.shape[3])
...
class BYOLEncoder(torch.nn.Module):
    ...
    def __init__(self):
        super(BYOLEncoder, self).__init__(feat_dimension=128, batch_size=128, num_heads=8, num_layers=8)
        ...
        self._audio_token = torch.randn(self._batch_size, 1, self._model_dimension)

        self._video_token = torch.randn(self._batch_size, 1, self._model_dimension)
    ...
    def get_temporal_modality_views(self, audio, video):
        a1, a2 = torch.split(audio, split_size=audio.shape[1]//2, dim=1) 
        v1, v2 = torch.split(video, split_size=video.shape[1]//2, dim=1)
        ...
        view1 = torch.stack((a1, v2))
        view2 = torch.stack((v1, a2))
        return view1, view2
    ...
    def forward(self, x):
        ...
        v1, v2 = self.get_temporal_modality_views(a,v)
